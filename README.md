...
### 多音源搜索与播放
- 五源并发搜索：网易云、QQ音乐、酷狗、酷我、咪咕，结果按评分排序合并
- 音源可配置：通过视觉控制台开关搜索源，实时生效
- VIP歌曲智能降级：自动通过 pyncmd / 咪咕 / 酷狗 / 酷我 等音源解锁完整版
- 五档音质选择：标准(128k) / 较高(192k) / 极高(320k) / 无损(FLAC) / Hi-Res

### 音乐下载
- 单曲下载：播放器控制台和搜索结果均可下载，支持品质选择并显示预估文件大小
- 批量下载：歌单列表支持选择式批量下载（全选/取消全选，高亮选中样式）
- 下载任务管理：实时进度显示、暂停/继续/取消、全部暂停/全部继续
- 下载记录持久化：退出程序后保留下载进度和记录，重启后可继续
- 按状态分类查看：全部 / 下载中 / 已暂停 / 已完成 / 失败 Tab 切换
- 试听片段检测：自动拒绝二三十秒的试听版，确保下载到完整歌曲
- 下载位置可配置：通过视觉控制台选择保存目录，持久保存
